...
- `Flavor::from_str(&str) -> Flavor` — case-insensitive; matches
  `mocha`/`macchiato`/`frappe`/`latte`; anything else (typo, empty) → `Mocha`.
  `frappe` accepted without the accent.
...
- `from_env` reads `PITWALL_THEME` and parses via `Flavor::from_str` (default
  Mocha when unset or unrecognized).
...
- `theme.rs`: `from_str` is case-insensitive and lenient — `"mocha"`, `"MOCHA"`,
  `"latte"` map correctly; `"garbage"` and `""` → `Mocha`. Sanity check that
  distinct flavors yield distinct `base` colors.
- `ui.rs` / `app.rs`: existing render tests updated to construct a
  `Palette::for_flavor(Flavor::Mocha)` and pass `&palette` in `View`. Content
  assertions (`ci-runner-1`, `idle`, `docker: unreachable`) still hold.
- `config.rs`: a test that an unset/garbage `PITWALL_THEME` yields `Flavor::Mocha`
  (asserted via the resulting `base` color, since `Flavor` need not derive `Eq`).

## Docs

`README.md` — add a `PITWALL_THEME` row (default `mocha`) to the env-var table and
note the four accepted flavor values.
